...
if ~ exist('simulationfiles/simVOI.mat', 'file') || REDO
    ...
    for vidx = 1 : nparams
        ...
        elseif strcmp(DATA_GENERATION, 'grid')
            ...
            tmpPE.lmu_eta = leta;
            tmpPE.lsigma_alpha =  norminv(exp(-5) ./ sqrt(2)); % Sigma is bounded in our case between 0 and -2 
...
for vidx = 1 : nvoxels
        ...
        rE0 = spm_vec(pE); 
        rC0 = spm_vec(pC);
        rE0([1]) = -4;  % setting all parameters of interest to small values except for lmu_eta, as 0 = no utility modulation
        rE0([3, 4]) =  norminv(exp(-5) ./ sqrt(2));
